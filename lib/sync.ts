...
import { applyConfig, defaultConfig, getConfig, getYear, type ScheduleConfig } from "@/data/schedule";
...
export async function syncNow() {
  ...
  try {
    ...
    await c.from("completions").upsert(
      jobs.map((j) => ({ homework_id: j.entry.id, user_id: me.id, done: j.entry.done, updated_at: new Date().toISOString() })),
      { onConflict: "homework_id,user_id" });

    writeOutbox([]); emit("idle");
  } catch (e) { console.warn(e); emit("error"); } finally { syncing = false; }
}
...
export async function pullAll() {
  ...
  try {
    ...
    const [{ data: hw }, { data: comps }] = await Promise.all([
      c.from("homework").select("*").eq("year", year),
...
export function subscribeAll() {
  ...
  const ch = c.channel("gm02")
    ...
    .on("postgres_changes", { event: "*", schema: "public", table: "schedules" }, () => void fetchSchedule())
    .subscribe();
...
export async function fetchSchedule(year?: number) {
  ...
  try {
    const { data } = await sb().from("schedules").select("*").eq("year", y).maybeSingle();
    if (data?.config) applyConfig({ ...(data.config as ScheduleConfig), year: y });
    else if (year) applyConfig(defaultConfig(y));
  } catch (e) { console.warn(e); }
}
...
export async function initSchedule() {
  const y = getYear();
  const { data } = await sb().from("schedules").select("year").eq("year", y).maybeSingle();
  if (!data) await saveSchedule(getConfig());
  else await fetchSchedule(y);
}
