...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<"tt" | "subj" | "duty">("tt");
  ...
  if (!ok) return (
    <div className="p-16 text-center">
      ...
      <Link href="/settings" className="mt-3 inline-block text-xs text-indigo-500 underline">กลับ</Link>
    </div>
  );
  ...
  const newYear = async () => {
    ...
    const next: ScheduleConfig = copy ? { ...getConfig(), year: y } : defaultConfig(y);
    setBusy(true);
    ...
    <main className="pb-32">
      <header className="rounded-b-3xl bg-gradient-to-br from-slate-700 to-slate-500 px-5 pb-5 pt-8 text-white">
        ...
        <div className="mt-3 flex items-center gap-2">
          <select value={cfg.year} onChange={(e) => void fetchSchedule(Number(e.target.value))}
            className="flex-1 rounded-xl bg-white/20 px-3 py-2 text-sm">
            {[...new Set([cfg.year, ...years])].sort((a,b)=>b-a).map((y) => (
              <option key={y} value={y} className="text-slate-800">ปีการศึกษา {y}</option>
            ))}
          </select>
          <button onClick={newYear} className="rounded-xl bg-white/25 px-3 py-2 text-xs font-semibold">+ ปีใหม่</button>
        </div>
      </header>

      <div className="flex gap-1.5 px-5 py-4">
        {([["tt","ตาราง"],["subj","รายวิชา"],["duty","เวรจด"]] as const).map(([k, l]) => (
          ...
          {Object.entries(cfg.subjects).map(([id, s]) => (
            <div key={id} className="card space-y-2 p-3">
              <div className="flex items-center gap-2">
                <input type="color" value={s.color} onChange={(e) => setSubj(id, { color: e.target.value })}
      ...
      {tab === "duty" && (
        <div className="card mx-5 space-y-2.5 p-4">
          <p className="text-[11px] text-slate-400">ตั้งชื่อคนที่รับผิดชอบจดการบ้านประจำแต่ละวัน</p>
          {ALL_DAYS.map((d) => (
      ...
      {msg && <p className="px-5 pt-4 text-center text-xs text-emerald-600">{msg}</p>}
      ...
      <div className="fixed inset-x-0 bottom-[68px] z-40 mx-auto max-w-md border-t border-slate-100 bg-white/95 px-5 py-3">
        <button onClick={save} disabled={busy}
          ...
          {busy ? "กำลังบันทึก…" : "💾 บันทึกตาราง (ทุกคนเห็นทันที)"}
        </button>
      </div>
    </main>
  );
