...
function Inner() {
  ...
  const update = (slot: DaySlot, patch: Partial<HomeworkEntry>) => {
    ...
    const updated = {
      ...
      ...patch,
      updatedAt: Date.now(),
      updatedBy: me?.nickname ?? "นักเรียน",
    };
  ...
  return (
    <main>
      <header className="sticky top-0 z-30 rounded-b-3xl bg-gradient-to-br from-indigo-500 to-sky-400 px-5 pb-5 pt-8 text-white shadow-lg shadow-indigo-100">
        <h1 className="text-center text-sm font-medium opacity-90">
          จดการบ้าน{duty && ` · เวรวันนี้: ${duty}`}
        </h1>

        <div className="mt-3 flex items-center gap-2">
          <button onClick={() => shiftDay(-1)} className="h-11 w-11 rounded-2xl bg-white/20 text-lg">
            ‹
          </button>
            ...
            <div key={slot.key} className={`card overflow-hidden ${open ? "ring-2 ring-indigo-200" : ""}`}>
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {!open && e.updatedBy && (
                    <p className="mt-0.5 text-[10px] text-indigo-400">👤 จดโดย {e.updatedBy}</p>
                  )}
                </div>
                ...
                <div className="space-y-3 border-t border-slate-100 bg-slate-50/70 p-4">
                  ...
                  <div className="flex items-end gap-3">
                    ...
                    <button
                      ...
                      className={`mb-0.5 shrink-0 rounded-xl px-3 py-2.5 text-xs font-medium ${
                  ...
                  {e.updatedBy && (
                    <p className="text-[10px] text-slate-400">👤 แก้ไขล่าสุดโดย {e.updatedBy}</p>
                  )}
                </div>
              )}
            </div>
      ...
      {slots.length > 0 && (
        <div className="fixed inset-x-0 bottom-[68px] z-40 mx-auto flex max-w-md gap-2.5 border-t border-slate-100 bg-white/90 px-5 py-3 backdrop-blur-xl">
          ...
          <button
            ...
            className="flex-1 rounded-2xl bg-gradient-to-r from-indigo-500 to-sky-400 py-3 text-sm font-semibold text-white"
          >
            🖼 สร้างการ์ด
          </button>
        </div>
